...
RainyDay.prototype.prepareCanvas = function() {
    ...
    jQuery(canvas).addClass(this.options.class);
    if(this.options.addAfter==true && this.options.element!=''){
jQuery(this.options.element).after(canvas);
    } else{
...
jQuery(document).ready(function(){
    var image = jQuery('.osc_esb_rainy_image');
    jQuery(image).each(function(ind,val){
        var pos = jQuery(val).position();
        var className=jQuery(val).attr('class');
        var drops=parseInt(jQuery(val).data('drops'));
        className= className.replace('osc_esb_rainy_image','');
        ...
        drops=drops!=''?drops:500;
        engine.gravity = engine.GRAVITY_NON_LINEAR;
        engine.trail = engine.TRAIL_DROPS;
        engine.rain([ [1, 2, drops] ]);
        engine.rain([ [3, 3, 0.88], [5, 5, 0.95], [6, 2, 1] ], 100);
